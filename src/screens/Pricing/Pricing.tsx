    ...
    <div className="bg-background w-full min-h-screen flex flex-col">
      ...
      <div className="flex-1 pt-16 sm:pt-20 md:pt-24 pb-4 sm:pb-6 px-3 sm:px-4 md:px-6 max-w-6xl mx-auto w-full">
        <div className="text-center mb-6 sm:mb-8 md:mb-10">
          <h1 className="text-foreground text-2xl sm:text-3xl md:text-4xl font-bold mb-2 flex items-center justify-center gap-2 sm:gap-3">
            <CreditCard className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-purple-400" /> {t('pricing')}
          </h1>
          <p className="text-muted-foreground text-sm sm:text-base">{t('pricingSubtitle')}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
          {plans.map((p) => (
            <div key={p.id} className={`rounded-xl sm:rounded-2xl border border-border bg-card p-4 sm:p-5 md:p-6 ${current===p.id? 'ring-2 ring-primary':''}`}>
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <h3 className="text-foreground text-lg sm:text-xl font-bold">{p.name}</h3>
                {p.id==='premium' && (
              ...
              <div className="text-2xl sm:text-3xl font-extrabold text-foreground mb-1">{p.price}<span className="text-xs sm:text-sm text-muted-foreground">/mo</span></div>
              <div className="text-muted-foreground mb-3 sm:mb-4 text-sm sm:text-base">{p.desc}</div>
              <div className="text-primary mb-3 sm:mb-4 text-sm sm:text-base">{t('tokensPerDay').replace('{count}', p.tokens.toLocaleString())}</div>
              <ul className="space-y-1.5 sm:space-y-2 mb-4 sm:mb-6">
                {p.features.map((f, i) => (
                  <li key={i} className="flex items-center gap-2 text-foreground text-xs sm:text-sm"><Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-400 flex-shrink-0" /> <span>{f}</span></li>
                ))}
              </ul>
              <button onClick={() => choosePlan(p.id as any)} className={`w-full py-2.5 sm:py-3 rounded-xl font-semibold text-sm sm:text-base transition-all ${current===p.id? 'bg-primary text-primary-foreground':'bg-secondary text-secondary-foreground hover:bg-secondary/80'}`}>
