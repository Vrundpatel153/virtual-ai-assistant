    ...
    <div className="bg-[#0a0b1e] w-full min-h-screen flex flex-col">
      ...
      <div className="flex-1 pt-16 sm:pt-20 md:pt-24 pb-4 sm:pb-6 px-3 sm:px-4 md:px-6">
        <div className="max-w-5xl mx-auto">
          <Card className="backdrop-blur-xl bg-gradient-to-br from-[#1e2139]/95 to-[#252844]/90 border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)] rounded-[16px] sm:rounded-[20px] md:rounded-[24px] overflow-hidden mb-4 sm:mb-6">
            <CardContent className="p-4 sm:p-6 md:p-8">
              <div className="flex flex-col md:flex-row items-center md:items-start gap-4 sm:gap-6">
                ...
                <div className="flex-1 text-center md:text-left min-w-0">
                  <h1 className="text-white text-xl sm:text-2xl md:text-3xl font-bold mb-2 truncate">{user?.name || 'User'}</h1>
                  <p className="text-gray-400 mb-3 sm:mb-4 text-sm sm:text-base">{user?.loginMethod === 'google' ? t('loginMethodGoogle') : t('loginMethodEmail')}</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 text-xs sm:text-sm">
                    ...
                    <div className="flex items-center gap-2 text-gray-300 justify-center md:justify-start sm:col-span-2 md:col-span-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
            {stats.map((stat, index) => (
              <Card
                key={index}
                className="backdrop-blur-xl bg-gradient-to-br from-[#1e2139]/95 to-[#252844]/90 border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)] rounded-[12px] sm:rounded-[16px] md:rounded-[20px] overflow-hidden"
              >
                <CardContent className="p-3 sm:p-4 md:p-6 text-center">
                  <div className={`${stat.color} flex items-center justify-center mb-1.5 sm:mb-2`}>
                    {stat.icon}
                  </div>
                  <div className="text-white text-xl sm:text-2xl md:text-3xl font-bold mb-0.5 sm:mb-1">{stat.value}</div>
                  <div className="text-gray-400 text-[10px] sm:text-xs md:text-sm leading-tight">{stat.label}</div>
                </CardContent>
              </Card>
            ))}
          </div>

          <Card className="backdrop-blur-xl bg-gradient-to-br from-[#1e2139]/95 to-[#252844]/90 border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)] rounded-[16px] sm:rounded-[20px] md:rounded-[24px] overflow-hidden">
            <CardContent className="p-4 sm:p-5 md:p-6">
              ...
              <div className="space-y-2 sm:space-y-3">
                {recentActivity.map((activity, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between p-2.5 sm:p-3 rounded-lg bg-[#2a2d4a]/50 hover:bg-[#2a2d4a] transition-colors"
                  >
                    <div className="flex-1 min-w-0 pr-2">
                      <p className="text-white text-xs sm:text-sm font-medium truncate">{activity.action}</p>
                      <p className="text-gray-400 text-[10px] sm:text-xs">{activity.time}</p>
                    </div>
